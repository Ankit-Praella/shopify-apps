...
class AjaxCart extends HTMLElement {
  constructor() {
    ...
    var data_cartamount = document.querySelector('[gift-variantid]').value;
    var cartTagData = document.querySelector('.item-data').getAttribute('data-product-tag');
    if(document.querySelector('.item-data').getAttribute('data-product-tag') === "special_product"){
      this.freeGift()
    }
  }
  ...
  ***/
  ...
  async freeGift() {
    var cartTag = document.querySelector('.item-data').getAttribute('data-product-tag');
    var giftVariantId = document.querySelector('[gift-variantid]').value;
    var cartContainsfreeGift = false;
    
    var cartData = await getCart();
    cartData.items.forEach((item)=>{
      if(item.id == giftVariantId){
        cartContainsfreeGift = true
      }
    })
    
    // checking condition && adding item in cart 
    if (cartContainsfreeGift == false && cartTag == "special_product") {
      let formdata = ({
        'items': [
          {
            'id': giftVariantId,
